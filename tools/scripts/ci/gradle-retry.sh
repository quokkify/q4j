...
# failure output shows an HTTP 429 (Too Many Requests) or 403 (Forbidden)
# response from a repository. 403 cannot reliably be classified as transient,
# so it is retried only within the same bounded attempt limit. Any other
# failure (404, DNS/connection error, compilation error, failing test, etc.)
# is not retried and fails immediately.
...
while true; do
  ...
  is_403=0
  grep -qE 'status code 403|403 Forbidden|Forbidden' "$output" && is_403=1

  if [ "$is_repository_failure" -ne 1 ] || { [ "$is_429" -ne 1 ] && [ "$is_403" -ne 1 ]; }; then
    rm -f "$output"
    exit "$status"
  fi
  ...
  echo "::warning::Gradle failed with a repository HTTP ${status_code} resolution error (attempt ${attempt}/${max_attempts}). Retrying in ${sleep_for}s..."
  rm -f "$output"
  sleep "$sleep_for"
