...
public class UiTableTest extends BaseTest {
  ...
  @TmsLink("UI_ID_3")
  @Test(description = "Verify local TABLE row lookup, cells, and ordered column values")
  public void testTable() {
    DelayedTablePage page = openPage();
    Firm firm = new Firm("Ernst Handel", "Roland Mendel", "Austria");

    var row = page.getTableRow(DelayedTablePage.Header.COMPANY, firm.company(), Duration.ofSeconds(5));
    row.verifyCell(DelayedTablePage.Header.CONTACT, firm.contact());
    row.verifyCell(DelayedTablePage.Header.COUNTRY, firm.country());
  ...
  @TmsLink("UI_ID_4")
  @Test(description = "Verify local TABLE map lookup and positional header mapping")
  ...
  @Test(description = "Verify DYNAMIC TABLE and FLEX TABLE exclude their header row")
