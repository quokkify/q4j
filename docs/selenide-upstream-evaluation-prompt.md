...
compatibility. “No external websites” applies to fixtures and browser access: use hermetic local
or classpath fixtures only, while local/classpath source is explicitly allowed for Selenide/API
research. Do not create a
...
whether each required issue evaluation/classification step was completed. The verdict is a single
standalone token on the final line (with no punctuation, explanation, or additional decision text).
```
